-- Maintainer's production project ONLY (column `date`, America/New_York). Bootstrap projects use
-- summary_date and their own time zone (bootstrap 002 section 4) and get their variant later.
-- Register D362. Audited 2026-09-14 (independent pre-deploy audit: APPROVE WITH CHANGES; the
-- three changes are applied here: type guard, EXECUTE grants, REVOKE on the invariant functions).
...
BEGIN
  IF p_metric_type LIKE 'HKCategoryTypeIdentifier%' OR p_metric_type = 'HKWorkoutTypeIdentifier' THEN
    RAISE EXCEPTION 'health4ai_compact_metric_v2: % is never compacted (categorical or workout payload)', p_metric_type;
  END IF;
...
GRANT EXECUTE ON FUNCTION public.health4ai_compact_metric_v2(uuid, text, date, interval, text) TO service_role;
...
-- Both invariant functions are SECURITY DEFINER with a p_user_id parameter: without these
-- lines PostgreSQL's default EXECUTE-to-PUBLIC would let any client read another user's
-- aggregate sums past RLS. Same rule as bootstrap 002 section 3.
REVOKE ALL ON FUNCTION public.health4ai_compaction_totals(uuid, text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.health4ai_compaction_totals(uuid, text) TO service_role;
REVOKE ALL ON FUNCTION public.health4ai_recompute_mismatches(uuid, text, date, text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.health4ai_recompute_mismatches(uuid, text, date, text) TO service_role;
