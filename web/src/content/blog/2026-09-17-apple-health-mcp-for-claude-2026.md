...
| Client surface | Claude.ai web connector is not Claude Code / Cursor MCP |
...
**HealthKit to your Supabase/Postgres to local MCP to Claude / Cursor / ChatGPT / Ollama**
...
If the own-DB path is what you want, the companion post [Apple Health to Postgres](/blog/apple-health-postgres) goes deeper on schema, RLS, and why Postgres is the wedge — not just "another sync app."
...
- "Summarize the last 7 days" to `get_health_summary`
- "HRV trend vs prior week" to `get_hrv_trend`
- "Sleep stages last night" to `get_sleep`
- "Anything odd about yesterday?" to `get_daily_snapshot` plus follow-ups
